...
export default function AuthModal() {
  ...
  const getStrengthLabel = () => {
    ...
    return { text: "Mạnh", color: "bg-brand-primary" };
  };
  ...
  return (
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white/90 dark:bg-zinc-950/90 backdrop-blur-3xl rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden animate-slide-up">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-1 bg-gradient-to-r from-transparent via-brand-primary to-transparent shadow-[0_0_20px_rgba(128,191,132,0.8)]" />
        ...
        <div className="p-8 md:p-10 flex flex-col max-h-[90vh] overflow-y-auto no-scrollbar">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              ...
              <p className="text-sm font-medium text-brand-base0 mt-1">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            {authMode === "REGISTER_CREDENTIALS" && (
                <div className="space-y-4 animate-fade-in">
                  {loginMethod === "EMAIL" ? (
                      ...
                      <div className="relative group">
                        <Phone className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-brand-primary transition-colors" size={20} />
                  ...
                  <div className="relative group">
                    <Lock className="absolute left-4 top-4 text-slate-400 group-focus-within:text-brand-primary transition-colors" size={20} />
                  ...
                  <div className="relative group">
                    <ShieldCheck className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-brand-primary transition-colors" size={20} />
            ...
            {authMode === "VERIFY_OTP" && (
                <div className="space-y-6 animate-slide-up">
                    <div className="p-4 rounded-2xl bg-brand-primary/10 border border-brand-primary/20 text-slate-700 dark:text-zinc-300 text-sm font-medium text-center">
                        Mã xác thực đã được gửi tới <br/><b className="text-brand-primary text-base">{loginMethod === "EMAIL" ? email : phone}</b>
                    </div>
                    ...
                    <div className="flex justify-between gap-1 sm:gap-2" onPaste={handleOtpPaste}>
                        {otpValues.map((value, index) => (
                            <input
                                ...
                                type="text"
                                maxLength={1}
                                className="w-9 h-12 md:w-11 md:h-14 text-center text-lg md:text-xl font-black bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-slate-900 dark:text-white focus:outline-none focus:border-brand-primary focus:ring-1 focus:ring-brand-primary/50 transition-all"
                                value={value}
                    ...
                    <div className="flex justify-between items-center px-2">
                        <span className="text-xs font-medium text-brand-base0 dark:text-zinc-400">Chưa nhận được mã?</span>
                        <button 
                            type="button" 
                            onClick={handleResendOtp}
                            disabled={countdown > 0}
                            className="text-xs font-bold text-brand-primary disabled:text-slate-400 dark:disabled:text-zinc-600 hover:underline transition-all"
                        >
            ...
            {authMode === "SETUP_PROFILE" && (
                <div className="space-y-4 animate-slide-up">
                    ...
                    <div className="pt-2">
                      <p className="text-[11px] text-brand-base0 font-medium">* Các thông tin như Ngày sinh, Giới tính có thể cập nhật sau.</p>
                    </div>
                </div>
            )}
          ...
          {(authMode === "LOGIN" || authMode === "REGISTER_CREDENTIALS") && (
              <>
                ...
                <div className="flex gap-3">
                  <button onClick={handleGoogleLogin} className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl font-bold text-sm text-slate-700 dark:text-white hover:bg-brand-base dark:hover:bg-white/10 transition-all group">
                  ...
                  <button 
                    ...
                    className="flex-1 flex items-center justify-center gap-2 py-3.5 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl font-bold text-sm text-slate-700 dark:text-white hover:bg-brand-base dark:hover:bg-white/10 transition-all group"
                  >
                    {loginMethod === "EMAIL" ? <Smartphone size={16} className="text-brand-base0 group-hover:text-brand-primary transition-colors"/> : <Mail size={16} className="text-brand-base0 group-hover:text-brand-primary transition-colors"/>}
                    {loginMethod === "EMAIL" ? "Số điện thoại" : "Dùng Email"}
                  </button>
                </div>

                <p className="mt-6 text-center text-sm font-medium text-brand-base0 dark:text-zinc-400">
                  {authMode === "LOGIN" ? "Chưa có định danh?" : "Đã là thành viên?"}
                  <button type="button" onClick={() => setAuthMode(authMode === "LOGIN" ? "REGISTER_CREDENTIALS" : "LOGIN")} className="ml-2 text-brand-primary font-black hover:underline underline-offset-4 transition-all">
