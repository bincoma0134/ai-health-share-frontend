...
export default function PartnerProfilePage() {
  ...
  const StatusBadge = ({ status }: { status: string }) => {
      const config: any = {
          'APPROVED': { label: 'Đang phát sóng', class: 'bg-brand-trust text-white' },
          ...
          'DELETED': { label: 'Đã gỡ', class: 'bg-brand-base0 text-white' } 
      };
  ...
  if (isLoading) return (
      <div className="h-[100dvh] w-full bg-brand-base dark:bg-zinc-950 flex flex-col items-center justify-center transition-colors duration-500">
          <Building2 className="text-brand-trust w-10 h-10 animate-pulse" />
          <p className="text-brand-base0 mt-4 text-xs font-black tracking-widest uppercase">Đang nạp không gian doanh nghiệp...</p>
      </div>
  );
    ...
    <div className="flex-1 relative h-[100dvh] flex flex-col bg-brand-base dark:bg-zinc-950 transition-colors duration-500 overflow-hidden font-be-vietnam">
      ...
      <div className="absolute top-0 w-full z-40 p-6 flex justify-end items-center bg-gradient-to-b from-brand-base dark:from-zinc-950 to-transparent pointer-events-none">
          <div className="flex items-center gap-3 pointer-events-auto">
            ...
            <button onClick={() => setIsNotifOpen(true)} className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-white/60 dark:bg-black/60 backdrop-blur-3xl border border-slate-200 dark:border-white/10 flex items-center justify-center text-brand-base0 hover:text-brand-trust shadow-lg transition-all"><Bell size={20} /></button>
          </div>
      </div>
          ...
          <div className="relative w-full h-48 md:h-64 bg-slate-200 dark:bg-zinc-900 group cursor-pointer overflow-hidden border-b border-slate-200 dark:border-white/5" onClick={() => coverInputRef.current?.click()}>
              {profileData?.profile?.cover_url ? (
                  ...
                  <div className="absolute inset-0 bg-gradient-to-br from-blue-900/40 to-cyan-900/40 flex items-center justify-center">
                      <div className="flex flex-col items-center opacity-50"><Building2 size={40} className="mb-2 text-brand-trust dark:text-blue-400"/> <span className="text-sm font-bold text-blue-700 dark:text-blue-300">Tải lên ảnh bìa cơ sở</span></div>
                  </div>
              )}
          ...
          <div className="max-w-6xl mx-auto px-6 md:px-12 pb-32 relative z-10">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-12 mb-10 animate-fade-in">
                ...
                <div className="relative shrink-0 -mt-16 md:-mt-20 group cursor-pointer" onClick={() => avatarInputRef.current?.click()}>
                  <div className="absolute -inset-1.5 bg-gradient-to-tr from-blue-400 to-brand-surface rounded-full blur-md opacity-40 group-hover:opacity-60 transition duration-1000"></div>
                  ...
                  <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 px-4 py-1.5 bg-gradient-to-r from-brand-trust to-brand-surface text-white text-[10px] font-black rounded-full shadow-xl border border-white/20 whitespace-nowrap uppercase flex items-center gap-1.5 tracking-widest z-20">
                ...
                <div className="flex-1 w-full pt-4 md:pt-6 text-center md:text-left">
                  ...
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-6">
                      <div>
                          <h1 className="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tighter flex items-center justify-center md:justify-start gap-2 mb-1 drop-shadow-md">
                              {profileData?.profile?.full_name || "Doanh nghiệp"} <BadgeCheck size={24} className="text-brand-trust" />
                          </h1>
                          <h2 className="text-base font-medium text-brand-base0 dark:text-zinc-400 tracking-tight">@{profileData?.profile?.username || "business_account"}</h2>
                      </div>
                      
                      <div className="flex items-center justify-center md:justify-end gap-3 mt-2 md:mt-0">
                          <button onClick={() => router.push(`/${profileData?.profile?.username}`)} className="px-6 py-3.5 bg-gradient-to-br from-brand-trust to-brand-surface text-white font-black rounded-2xl hover:shadow-lg hover:shadow-brand-trust/30 transition-all flex items-center gap-2 active:scale-95 text-sm uppercase tracking-widest">
                              <Eye size={18} strokeWidth={3} /> Xem công khai
                          </button>
                          <button onClick={handleShareProfile} className="p-3.5 bg-white/40 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-white rounded-xl hover:bg-blue-50 dark:hover:bg-brand-trust/10 hover:text-brand-trust transition-all shadow-sm active:scale-90">
                              <Share2 size={18} />
                          </button>
                  ...
                  <div className="flex items-center justify-center md:justify-start gap-8 mb-6">
                      ...
                      <div className="w-[1px] h-8 bg-slate-200 dark:bg-white/10"></div>
                      <div className="flex items-center gap-2 group cursor-pointer">
                          <span className="text-xl md:text-2xl font-black text-brand-trust transition-colors">{profileData?.profile?.reputation_points || 92}</span>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 relative items-start border-t border-slate-200 dark:border-white/10 pt-10">
                <div className="lg:col-span-8">
                    <div className="flex justify-start gap-8 border-b border-slate-200 dark:border-white/10 pb-4 overflow-x-auto no-scrollbar">
                        <button onClick={() => setActiveTab('services')} className={`flex items-center gap-2 text-xs font-black transition-all whitespace-nowrap ${activeTab === 'services' ? 'text-brand-trust border-b-2 border-brand-trust pb-4 -mb-5' : 'text-slate-400 hover:text-slate-600 dark:hover:text-zinc-300'}`}>
                            <Package size={16}/> QUẢN LÝ DỊCH VỤ
                        </button>
                        <button onClick={() => setActiveTab('studio')} className={`flex items-center gap-2 text-xs font-black transition-all whitespace-nowrap ${activeTab === 'studio' ? 'text-brand-trust border-b-2 border-brand-trust pb-4 -mb-5' : 'text-slate-400 hover:text-slate-600 dark:hover:text-zinc-300'}`}>
                            <Video size={16}/> STUDIO VIDEO
                        </button>
                        <button onClick={() => setActiveTab('reviews')} className={`flex items-center gap-2 text-xs font-black transition-all whitespace-nowrap ${activeTab === 'reviews' ? 'text-brand-trust border-b-2 border-brand-trust pb-4 -mb-5' : 'text-slate-400 hover:text-slate-600 dark:hover:text-zinc-300'}`}>
                            <Star size={16}/> LỊCH SỬ ĐÁNH GIÁ
                        </button>
                        <button onClick={() => setActiveTab('info')} className={`flex items-center gap-2 text-xs font-black transition-all whitespace-nowrap ${activeTab === 'info' ? 'text-brand-trust border-b-2 border-brand-trust pb-4 -mb-5' : 'text-slate-400 hover:text-slate-600 dark:hover:text-zinc-300'}`}>
                        ...
                        {activeTab === 'services' && (
                             <div className="animate-fade-in space-y-6">
                                 <div className="flex justify-between items-center mb-6">
                                     ...
                                     <button onClick={() => setIsAddModalOpen(true)} className="px-5 py-2.5 bg-brand-trust text-white font-bold text-xs rounded-xl shadow-lg hover:bg-brand-trust transition-all active:scale-95 flex items-center gap-2">
                                             ...
                                             <div key={svc.id} onClick={() => setExpandedService(svc)} className="bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl rounded-[2rem] border border-slate-200 dark:border-white/10 overflow-hidden shadow-sm hover:shadow-2xl hover:border-brand-trust/50 hover:-translate-y-1 transition-all duration-300 group flex flex-col relative cursor-pointer">
                                                <div className="relative h-44 overflow-hidden shrink-0 bg-slate-100 dark:bg-black">
                                                    ...
                                                    <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                                        ...
                                                        }} className="p-2.5 bg-white/90 dark:bg-black/80 backdrop-blur-md text-brand-trust dark:text-blue-400 rounded-xl shadow-lg hover:bg-blue-50 dark:hover:bg-white/10 transition-colors"><Edit3 size={16}/></button>
                                                ...
                                                <div className="p-6 flex-1 flex flex-col">
                                                    <h4 className="font-black text-lg text-slate-900 dark:text-white line-clamp-2 mb-2 group-hover:text-brand-trust transition-colors">{svc.service_name}</h4>
                                                    <p className="text-xs font-medium text-brand-base0 dark:text-zinc-400 line-clamp-2 mb-4 flex-1">{svc.description}</p>
                                                    ...
                                                    <div className="flex items-center justify-between border-t border-slate-100 dark:border-white/5 pt-4">
                                                        ...
                                                        <span className="text-xl font-black text-brand-trust dark:text-blue-400">{parseFloat(svc.price).toLocaleString('vi-VN')} đ</span>
                                                    </div>
                                                </div>
                                             </div>
                                         ))}
                        ...
                        {activeTab === 'studio' && (
                             <div className="animate-fade-in space-y-6">
                                 <div className="flex justify-between items-center mb-6">
                                     ...
                                     <button onClick={() => setIsAddVideoModalOpen(true)} className="px-5 py-2.5 bg-brand-trust text-white font-bold text-xs rounded-xl shadow-lg hover:bg-brand-trust transition-all active:scale-95 flex items-center gap-2">
                                 ...
                                 {myVideos.length === 0 ? (
                                     <div className="text-center py-20 bg-white/40 dark:bg-white/5 border border-dashed border-slate-200 dark:border-white/10 rounded-[2rem]">
                                         ...
                                         <p className="text-brand-base0 font-bold">Chưa có video nào được tải lên Studio.</p>
                                     </div>
                                 ) : (
                                     <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6">
                                         {myVideos.map(vid => (
                                             <div key={vid.id} onClick={() => setExpandedVideo(vid)} className="aspect-[9/16] rounded-[2rem] overflow-hidden bg-black relative group shadow-sm hover:shadow-2xl border border-slate-200 dark:border-white/10 hover:border-brand-trust/50 cursor-pointer hover:-translate-y-1 transition-all duration-300">
                                                 ...
                                                 <div className="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                                                    <button onClick={(e) => { e.stopPropagation(); setEditingVideo(vid); setIsEditVideoModalOpen(true); }} className="p-2.5 bg-white/90 dark:bg-black/80 backdrop-blur-md text-brand-trust dark:text-blue-400 rounded-xl shadow-lg hover:bg-blue-50 dark:hover:bg-white/10 transition-colors"><Edit3 size={16}/></button>
                                                 ...
                                                 <div className="absolute inset-x-0 bottom-0 p-5 bg-gradient-to-t from-black/90 via-black/40 to-transparent pointer-events-none">
                                                     ...
                                                     {vid.price ? (
                                                        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-brand-trust/20 backdrop-blur-md border border-blue-400/30 rounded-lg">
                                ...
                                <div className="bg-white/60 dark:bg-zinc-900/50 backdrop-blur-2xl p-8 rounded-[3rem] border border-slate-200 dark:border-white/10 shadow-lg">
                                    ...
                                    <div className="flex items-center gap-6 mb-10">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs font-bold text-brand-base0 uppercase">Dựa trên 124 lượt đánh giá</p>
                                        </div>
                                    </div>
                                    <div className="space-y-6">
                                        ...
                                        {[
                                            ...
                                            <div key={idx} className="p-5 bg-brand-base dark:bg-white/5 rounded-[2rem] border border-slate-100 dark:border-white/5 transition-all hover:scale-[1.01]">
                                                ...
                                                <p className="text-xs text-brand-base0 dark:text-zinc-400 font-medium italic leading-relaxed">"{fb.comment}"</p>
                                            </div>
                                        ))}
                                    </div>
                                    <button className="w-full mt-8 py-4 bg-slate-100 dark:bg-white/5 text-brand-base0 dark:text-zinc-400 font-black text-[10px] uppercase tracking-widest rounded-2xl hover:bg-slate-200 dark:hover:bg-white/10 transition-all">
                                        Xem tất cả đánh giá
                                    </button>
                                </div>
                            </div>
                                ...
                                <div className="bg-white/60 dark:bg-zinc-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2rem] border border-slate-200 dark:border-white/10 shadow-lg space-y-6 relative">
                                    {isUploadingImage && (
                                        <div className="absolute inset-0 bg-white/50 dark:bg-black/50 backdrop-blur-sm z-50 rounded-[2rem] flex items-center justify-center">
                                            <div className="w-8 h-8 border-4 border-brand-trust border-t-transparent rounded-full animate-spin"></div>
                                        </div>
                                    )}
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <input type="text" className="w-full bg-slate-100/50 dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-2xl px-5 py-4 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:border-brand-trust shadow-inner" value={editForm.full_name} onChange={e => setEditForm({...editForm, full_name: e.target.value})} />
                                    </div>
                                    ...
                                    <div className="pt-4 border-t border-slate-200 dark:border-white/10">
                                        <div className="flex items-center justify-between mb-4">
                                            ...
                                            <button onClick={addSocial} className="text-brand-trust hover:text-brand-trust font-black text-xs flex items-center gap-1 bg-blue-50 dark:bg-brand-trust/10 px-3 py-1.5 rounded-lg"><Plus size={14}/> Thêm Link</button>
                                        </div>
                                            ...
                                            {socials.map((social, idx) => (
                                                <div key={idx} className="flex flex-col sm:flex-row items-center gap-3 bg-brand-base dark:bg-white/5 p-3 rounded-2xl border border-slate-200 dark:border-white/5">
                                                    ...
                                                    <div className="w-full sm:flex-1 flex items-center gap-2">
                                                        <input type="url" placeholder="https://..." value={social.url} onChange={(e) => updateSocial(idx, 'url', e.target.value)} className="w-full bg-white dark:bg-black/40 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:border-brand-trust" />
                                    ...
                                    <button onClick={handleUpdateProfile} disabled={isUpdating} className="w-full py-5 bg-brand-trust hover:bg-blue-700 text-white font-black rounded-2xl shadow-xl active:scale-95 transition-all text-xs tracking-widest uppercase">LƯU THÔNG TIN HỒ SƠ</button>
                                </div>
                            </div>
                        )}
                    </div>
                ...
                <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-8 lg:pt-[52px]">
                    <div className="bg-white/60 dark:bg-zinc-900/50 backdrop-blur-2xl rounded-[3rem] p-8 border border-slate-200 dark:border-white/10 relative overflow-hidden shadow-lg flex flex-col">
                        <div className="flex items-center gap-2 text-brand-trust font-black text-xs mb-4 uppercase tracking-widest">
        ...
        <div className="fixed inset-0 z-[100] flex justify-center items-center p-4">
            ...
            <div className="relative w-full max-w-xl bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-2xl animate-slide-up overflow-hidden flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-white/5 bg-brand-base/50 dark:bg-black/20">
                    <h3 className="font-black text-lg text-slate-900 dark:text-white flex items-center gap-2"><Package size={20} className="text-brand-trust"/> Thêm Dịch Vụ Mới</h3>
                    <button onClick={() => setIsAddModalOpen(false)} className="p-2 bg-slate-200/50 dark:bg-white/5 rounded-full text-brand-base0"><X size={16}/></button>
                </div>
                <form onSubmit={handleAddService} className="p-6 space-y-5 overflow-y-auto no-scrollbar">
                    ...
                    <div>
                        ...
                        <input type="text" className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:border-brand-trust" value={newService.service_name} onChange={e => setNewService({...newService, service_name: e.target.value})} required />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-1 block">Giá tiền (VNĐ)</label>
                            <input type="number" className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-black text-brand-trust focus:outline-none focus:border-brand-trust" value={newService.price} onChange={e => setNewService({...newService, price: e.target.value})} required />
                        </div>
                        <div>
                            <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-1 block">Tags (cách nhau phẩy)</label>
                            <input type="text" placeholder="VD: Mới nhất, Hot" className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium text-slate-900 dark:text-white focus:outline-none focus:border-brand-trust" value={newService.tags} onChange={e => setNewService({...newService, tags: e.target.value})} />
                        </div>
                    </div>
                    <div>
                        <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-1 block">Mô tả chi tiết</label>
                        <textarea rows={3} className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium text-slate-900 dark:text-white focus:outline-none focus:border-brand-trust resize-none" value={newService.description} onChange={e => setNewService({...newService, description: e.target.value})} />
                    </div>
                    <button type="submit" disabled={isUploading} className="w-full py-4 mt-2 bg-brand-trust hover:bg-blue-700 text-white font-black rounded-xl shadow-lg active:scale-95 transition-all uppercase tracking-widest text-xs flex justify-center items-center gap-2">
      ...
      {isEditModalOpen && editingService && (
        <div className="fixed inset-0 z-[100] flex justify-center items-center p-4">
            ...
            <div className="relative w-full max-w-xl bg-white/90 dark:bg-zinc-900/90 backdrop-blur-3xl rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-2xl animate-slide-up overflow-hidden flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-white/5 bg-brand-base/50 dark:bg-black/20">
                    <h3 className="font-black text-lg text-slate-900 dark:text-white flex items-center gap-2"><Edit3 size={20} className="text-brand-trust"/> Sửa Dịch Vụ</h3>
                    <button onClick={() => setIsEditModalOpen(false)} className="p-2 bg-slate-200/50 dark:bg-white/5 rounded-full text-brand-base0"><X size={16}/></button>
                </div>
                <form onSubmit={handleEditService} className="p-6 space-y-5 overflow-y-auto no-scrollbar">
                    <div>
                        <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-2 block">Tên dịch vụ</label>
                        <input type="text" className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:border-brand-trust" value={editingService.service_name} onChange={e => setEditingService({...editingService, service_name: e.target.value})} required />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-2 block">Giá tiền (VNĐ)</label>
                            <input type="number" className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-black text-brand-trust focus:outline-none focus:border-brand-trust" value={editingService.price} onChange={e => setEditingService({...editingService, price: e.target.value})} required />
                        </div>
                        <div>
                            <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-2 block">Nhãn dán (cách nhau phẩy)</label>
                            <input type="text" placeholder="VD: Mới nhất, Hot" className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium text-slate-900 dark:text-white focus:outline-none focus:border-brand-trust" 
                    ...
                    <div>
                        <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-2 block">Mô tả chi tiết</label>
                        <textarea rows={3} className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium text-slate-900 dark:text-white focus:outline-none focus:border-brand-trust resize-none" value={editingService.description} onChange={e => setEditingService({...editingService, description: e.target.value})} />
                    </div>
                    <button type="submit" disabled={isUploading} className="w-full py-4 mt-2 bg-brand-trust hover:bg-blue-700 text-white font-black rounded-xl shadow-lg active:scale-95 transition-all uppercase tracking-widest text-xs flex justify-center items-center gap-2">
      ...
      {isAddVideoModalOpen && (
        <div className="fixed inset-0 z-[100] flex justify-center items-center p-4">
            ...
            <div className="relative w-full max-w-[800px] bg-white dark:bg-zinc-900 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-2xl animate-slide-up overflow-hidden flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-white/5 bg-brand-base/50 dark:bg-black/20">
                    <h3 className="font-black text-lg text-slate-900 dark:text-white flex items-center gap-2"><Video size={20} className="text-brand-trust"/> Tải Video Lên Studio</h3>
      ...
      {isEditVideoModalOpen && editingVideo && (
        <div className="fixed inset-0 z-[100] flex justify-center items-center p-4">
            ...
            <div className="relative w-full max-w-xl bg-white/90 dark:bg-zinc-900/90 backdrop-blur-3xl rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-2xl animate-slide-up overflow-hidden flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center p-6 border-b border-slate-100 dark:border-white/5 bg-brand-base/50 dark:bg-black/20">
                    <h3 className="font-black text-lg text-slate-900 dark:text-white flex items-center gap-2"><Edit3 size={20} className="text-brand-trust"/> Sửa Thông Tin Video</h3>
                    <button onClick={() => setIsEditVideoModalOpen(false)} className="p-2 bg-slate-200/50 dark:bg-white/5 rounded-full text-brand-base0"><X size={16}/></button>
                </div>
                <form onSubmit={handleEditVideo} className="p-6 space-y-5 overflow-y-auto no-scrollbar">
                    <div>
                        <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-2 block">Tên video</label>
                        <input type="text" className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-bold text-slate-900 dark:text-white focus:outline-none focus:border-brand-trust" value={editingVideo.title} onChange={e => setEditingVideo({...editingVideo, title: e.target.value})} required />
                    </div>
                    <div>
                        <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-2 block">Giá tham khảo (VNĐ)</label>
                        <input type="number" className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-black text-brand-trust focus:outline-none focus:border-brand-trust" value={editingVideo.price || ''} onChange={e => setEditingVideo({...editingVideo, price: e.target.value})} />
                    </div>
                    <div>
                        <label className="text-[10px] font-black text-brand-base0 uppercase tracking-widest mb-2 block">Mô tả nội dung</label>
                        <textarea rows={3} className="w-full bg-slate-100/50 dark:bg-black/50 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm font-medium text-slate-900 dark:text-white focus:outline-none focus:border-brand-trust resize-none" value={editingVideo.content} onChange={e => setEditingVideo({...editingVideo, content: e.target.value})} />
                    </div>
                    <button type="submit" disabled={isStudioUploading} className="w-full py-4 mt-2 bg-brand-trust text-white font-black rounded-xl shadow-lg active:scale-95 transition-all uppercase tracking-widest text-xs flex justify-center items-center gap-2">
        ...
        <div className="fixed inset-0 z-[140] flex justify-center items-center p-4 md:p-6">
          ...
          <div className="relative w-full max-w-4xl bg-white dark:bg-[#0a0a0a] rounded-[2.5rem] overflow-hidden flex flex-col md:flex-row shadow-[0_20px_60px_rgba(0,0,0,0.2)] dark:shadow-[0_20px_60px_rgba(0,0,0,0.9)] border border-slate-200 dark:border-white/10 animate-slide-up max-h-[90vh]">
            ...
            <div className="flex-1 p-6 md:p-10 flex flex-col overflow-y-auto no-scrollbar relative">
                ...
                <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-brand-trust/10 border border-brand-trust/20 rounded-full text-[10px] font-bold text-brand-trust dark:text-blue-400 mb-4 uppercase tracking-wider w-max">
                ...
                <div className="mt-auto pt-6 border-t border-slate-100 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                    <div>
                        ...
                        <p className="text-3xl font-black text-brand-trust dark:text-blue-400">{Number(expandedService.price).toLocaleString()} <span className="text-base text-brand-base0">đ</span></p>
                    </div>
                    ...
                    <button 
                        ...
                        className="w-full sm:w-auto px-8 py-4 bg-slate-900 dark:bg-white hover:bg-brand-trust dark:hover:bg-brand-trust text-white dark:text-zinc-950 dark:hover:text-white font-black rounded-2xl shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest text-xs"
                    >
          ...
          <div className="relative w-full h-full flex items-center justify-center pointer-events-none p-4 md:p-10">
            <div className="relative h-full max-h-[92vh] aspect-[9/16] bg-black rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)] border border-white/10 animate-slide-up pointer-events-auto">
                ...
                <button onClick={() => { setEditingVideo(expandedVideo); setExpandedVideo(null); setIsCommentModalOpen(false); setIsEditVideoModalOpen(true); }} 
                        className="absolute top-6 right-6 z-30 group flex items-center gap-2 px-4 py-2.5 rounded-full bg-white/10 backdrop-blur-xl border border-white/20 text-white hover:bg-brand-trust hover:border-blue-400 hover:scale-105 transition-all shadow-xl active:scale-95">
                ...
                <div className="absolute bottom-[40px] left-6 z-20 max-w-[80%] pointer-events-auto animate-slide-up">
                    <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-brand-primary/20 backdrop-blur-md border border-brand-primary/30 rounded-full text-[9px] font-black text-brand-primary mb-4 uppercase tracking-widest">
                    ...
                    {expandedVideo.price && (
                      <div className="flex items-center gap-3 pl-1.5 pr-6 py-2 bg-white/10 backdrop-blur-xl border border-white/20 text-white rounded-full w-max shadow-2xl">
                        ...
                        <div className="flex flex-col"><span className="text-[8px] font-black text-brand-primary uppercase leading-none mb-0.5">Giá tham khảo</span><span className="font-black text-sm leading-none">{parseFloat(expandedVideo.price).toLocaleString()} đ</span></div>
                      </div>
                    )}
                </div>
